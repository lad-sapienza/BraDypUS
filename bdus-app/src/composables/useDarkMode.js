 ...
 *
 * Backed by a three-way `mode` ('light' | 'dark' | 'system') — 'system'
 * follows the OS preference live (via a matchMedia listener, not just a
 * one-time read) and is what a fresh install defaults to. `toggle()` cycles
 * light -> dark -> system -> light, so all three are reachable from the
 * single topbar/login button (AppLayout.vue / LoginView.vue render a third,
 * distinct icon for 'system' rather than collapsing it into light/dark).
 *
...
// Exported so AppLayout.vue/LoginView.vue can label the toggle button with
// what clicking it will switch *to*, without duplicating the cycle order.
export const NEXT_MODE = { light: 'dark', dark: 'system', system: 'light' }

export function useDarkMode() {
  function toggle() {
    mode.value = NEXT_MODE[mode.value]
  }
